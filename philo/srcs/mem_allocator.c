...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/15 17:47:03 by earutiun          #+#    #+#             */
/*   Updated: 2025/11/15 17:47:05 by earutiun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_curph	*mem_allocator(int *args, t_philo *ph_struct, void *mem[])
{
	...
	if (!(alloc_threads(args[0], mem)))
		return (mem_cleaner(mem));
	locks = alloc_mutexes(args[0], mem);
	if (!locks)
		return (mem_cleaner(mem));
	philosophers = alloc_phils(args[0], mem);
	if (!philosophers)
		return (mem_cleaner(mem));
...
static pthread_mutex_t	*alloc_mutexes(int number_of_philosophers, void *mem[])
{
	...
	locks = (pthread_mutex_t *)
		malloc(number_of_philosophers * sizeof(pthread_mutex_t));
	if (!locks)
	{
